...
use_grad_scalar = (dtype == torch.float16)
scaler = torch.cuda.amp.GradScaler("cuda",enabled=use_grad_scalar)
print(f"Gradient scalar enabled: {use_grad_scalar}")

# optimizer
optimizer = model.configure_optimizers(opt_name, weight_decay, learning_rate, hyper_lr, (beta1, beta2), device_type)
if init_from == 'resume':
...
if wandb_log and master_process:
    import wandb
    wandb.init(project=wandb_project, name=f"{opt_name}_{learning_rate}_{hyper_lr}", config=config)

file_name = 'results/{}/{:+.0e}_{:+.0e}/{}.csv'.format(wandb_project, learning_rate, hyper_lr, opt_name)
os.makedirs(os.path.dirname(file_name), exist_ok=True)
print('Output file: {}'.format(file_name))
f = open(file_name, 'w') if csv_log else None
writer = csv.writer(f) if csv_log and f else None
